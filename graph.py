...
class Graph:
    ...
    def path_visualizer(self, nodes, goal, path=[], counter=0, root_element=None):
        try:
            ...
            if empty_lists_counter == len(list(nodes.values())) - 1:
                ...
                for i in nodes[element]:
                    ...
                    if goal in nodes[i]:
                        ...
                        original_root_element = list(nodes.keys())[0]
                        if path[0] != root_element:
                            path = [original_root_element] +  path + [goal]
                            break
                        else:
                            path = [original_root_element] + path + [goal]
                            break
                        ...
                        for index in range(len(list(nodes.keys()))):
                            if list(nodes.keys())[index] == i:
                                ...
                                if root_node_occurances_counter > 0:
                                    counter = index 
                                    return self.path_visualizer(nodes, goal, path, counter, root_element)
                                else:
                                    counter += 1
                                    return self.path_visualizer(nodes, goal, path, counter, root_element=list(nodes.keys())[counter])
...
print(test.path_visualizer(nodes, 2, root_element=list(nodes.keys())[0]))
